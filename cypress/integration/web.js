...
describe("create new registration", () => {
  beforeEach(() => {
    ...
    PageAktivasi.aktivasiAkun().click();
    PageRegister.btnPageDaftar();
    PageRegister.ddlTemanYoutap().click();
    PageRegister.btnDaftar().click();
  });

  it("registrasi no baru", () => {
    PageRegister.frmNoHp().type(nohp);
    PageRegister.btnLanjutkan().click();
    PageRegister.frmName().type(name);
    PageRegister.frmBusiness().type(location);
    PageRegister.frmCategory().click();
    PageRegister.lstCategory(number).click({ force: true });
    PageRegister.btnLanjutkan().click();
  ...
  it("registrasi no baru with less 4 character", () => {
    PageRegister.frmNoHp().type("824");
    PageRegister.btnLanjutkan().click();

    //assert
    PageRegister.msgError().should("have.text", "Bad Request");
  });
});
